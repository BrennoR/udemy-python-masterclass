...
db.execute("CREATE TABLE IF NOT EXISTS history (time TIMESTAMP NOT NULL, "
           ...
           "zone INTEGER NOT NULL, PRIMARY KEY (time, account))")     # Composite Key
...
class Account(object):
    ...
    def _save_update(self, amount):
        ...
        time, zone = Account._current_time()    # <- unpacking return tuple
        pickled_zone = pickle.dumps(zone)

        db.execute("UPDATE accounts SET balance = ? WHERE (name = ?)", (new_balance, self.name))
        db.execute("INSERT INTO history VALUES (?, ?, ?, ?)", (time, self.name, amount, pickled_zone))
        db.commit()
    ...
    def show_balance(self):
        print("Balance of account {} is {:.2f}".format(self.name, self._balance / 100))
...
if __name__ == "__main__":
    ...
    bobs = Account("Bobs", 1000)
    sterling = Account("Sterling", 5000)
